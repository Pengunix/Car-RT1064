/*********************************************************************************************************************
 * RT1064DVL6A Opensourec Library 即（RT1064DVL6A 开源库）是一个基于官方 SDK
...
int16 mpu6050_gyro_x = 0, mpu6050_gyro_y = 0,
      mpu6050_gyro_z = 0; // 三轴陀螺仪数据      gyro (陀螺仪)
int16 mpu6050_acc_x = 0, mpu6050_acc_y = 0,
      mpu6050_acc_z = 0; // 三轴加速度计数据    acc (accelerometer 加速度计)

#if MPU6050_USE_SOFT_IIC
static soft_iic_info_struct mpu6050_iic_struct;

#define mpu6050_write_register(reg, data)                                      \
  (soft_iic_write_8bit_register(&mpu6050_iic_struct, (reg), (data)))
#define mpu6050_read_register(reg)                                             \
  (soft_iic_read_8bit_register(&mpu6050_iic_struct, (reg)))
#define mpu6050_read_registers(reg, data, len)                                 \
  (soft_iic_read_8bit_registers(&mpu6050_iic_struct, (reg), (data), (len)))
#else
#define mpu6050_write_register(reg, data)                                      \
  (iic_write_8bit_register(MPU6050_IIC, MPU6050_DEV_ADDR, (reg), (data)))
#define mpu6050_read_register(reg)                                             \
  (iic_read_8bit_register(MPU6050_IIC, MPU6050_DEV_ADDR, (reg)))
#define mpu6050_read_registers(reg, data, len)                                 \
  (iic_read_8bit_registers(MPU6050_IIC, MPU6050_DEV_ADDR, (reg), (data), (len)))
#endif
...
// 返回参数     void
// 使用示例     float data = mpu6050_acc_transition(mpu6050_acc_x); // 单位为
// g(m/s^2) 备注信息
//-------------------------------------------------------------------------------------------------------------------
float mpu6050_acc_transition(int16 acc_value) {
  ...
  case 0x00:
    ...
    break; // 0x00 加速度计量程为:±2 g    获取到的加速度计数据 除以 16384
           // 可以转化为带物理单位的数据 (g 代表重力加速度 物理学名词 一般情况下
           // g 取 9.8 m/s^2 为标准值)
  case 0x08:
    acc_data = (float)acc_value / 8192;
    break; // 0x08 加速度计量程为:±4 g    获取到的加速度计数据 除以 8192
           // 可以转化为带物理单位的数据 (g 代表重力加速度 物理学名词 一般情况下
           // g 取 9.8 m/s^2 为标准值)
  case 0x10:
    acc_data = (float)acc_value / 4096;
    break; // 0x10 加速度计量程为:±8 g    获取到的加速度计数据 除以 4096
           // 可以转化为带物理单位的数据 (g 代表重力加速度 物理学名词 一般情况下
           // g 取 9.8 m/s^2 为标准值)
  case 0x18:
    acc_data = (float)acc_value / 2048;
    break; // 0x18 加速度计量程为:±16g    获取到的加速度计数据 除以 2048
           // 可以转化为带物理单位的数据 (g 代表重力加速度 物理学名词 一般情况下
           // g 取 9.8 m/s^2 为标准值)
  default:
    break;
...
// 返回参数     void
// 使用示例     float data = mpu6050_gyro_transition(mpu6050_gyro_x); //
// 单位为°/s 备注信息
//-------------------------------------------------------------------------------------------------------------------
float mpu6050_gyro_transition(int16 gyro_value) {
  ...
  case 0x00:
    ...
    break; // 0x00 陀螺仪量程为:±250 dps     获取到的陀螺仪数据除以 131
           // 可以转化为带物理单位的数据，单位为：°/s
  case 0x08:
    gyro_data = (float)gyro_value / 65.5f;
    break; // 0x08 陀螺仪量程为:±500 dps     获取到的陀螺仪数据除以 65.5
           // 可以转化为带物理单位的数据，单位为：°/s
  case 0x10:
    gyro_data = (float)gyro_value / 32.8f;
    break; // 0x10 陀螺仪量程为:±1000dps     获取到的陀螺仪数据除以 32.8
           // 可以转化为带物理单位的数据，单位为：°/s
  case 0x18:
    gyro_data = (float)gyro_value / 16.4f;
    break; // 0x18 陀螺仪量程为:±2000dps     获取到的陀螺仪数据除以 16.4
           // 可以转化为带物理单位的数据，单位为：°/s
  default:
    break;
  }
...
uint8 mpu6050_init(void) {
...
#else
  iic_init(MPU6050_IIC, MPU6050_DEV_ADDR, MPU6050_IIC_SPEED, MPU6050_SCL_PIN,
           MPU6050_SDA_PIN);
#endif
  ...
  do {
    ...
    // ACCEL_CONFIG寄存器
    // 设置为:0x00 加速度计量程为:±2g          获取到的加速度计数据 除以16384
    // 可以转化为带物理单位的数据，单位：g(m/s^2) 设置为:0x08 加速度计量程为:±4g
    // 获取到的加速度计数据 除以8192 可以转化为带物理单位的数据，单位：g(m/s^2)
    // 设置为:0x10 加速度计量程为:±8g          获取到的加速度计数据 除以4096
    // 可以转化为带物理单位的数据，单位：g(m/s^2) 设置为:0x18
    // 加速度计量程为:±16g         获取到的加速度计数据 除以2048
